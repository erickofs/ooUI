"""Settings tab — backend URL, local port, API key management, and ooProxy Home."""
...
class SettingsTab(QWidget):
    """Configuration panel for backend URL, port, API keys, and ooProxy Home."""
    ...
    def _setup_ui(self) -> None:
        ...
        home_row.addWidget(QLabel(t("settings.ooproxy_home_label")))
        self._home_input = QLineEdit()
        self._home_input.setPlaceholderText(t("settings.ooproxy_home_placeholder"))
        self._home_input.setReadOnly(True)
        home_row.addWidget(self._home_input, 1)
        self._btn_browse_home = QPushButton(t("settings.ooproxy_home_browse"))
        self._btn_browse_home.setCursor(Qt.CursorShape.PointingHandCursor)
        self._btn_browse_home.setFixedHeight(32)
        self._btn_browse_home.clicked.connect(self._on_browse_home)
        home_row.addWidget(self._btn_browse_home)
        bg_layout.addLayout(home_row)
        ...
        self._keys_table.horizontalHeader().setSectionResizeMode(0, QHeaderView.ResizeMode.Stretch)
        self._keys_table.horizontalHeader().setSectionResizeMode(1, QHeaderView.ResizeMode.ResizeToContents)
        self._keys_table.horizontalHeader().setSectionResizeMode(2, QHeaderView.ResizeMode.ResizeToContents)
    ...
    def _on_settings_loaded(self, settings) -> None:
        ...
        self._home_input.setText(settings.ooproxy_home)
    ...
    def _on_browse_home(self) -> None:
        current = self._home_input.text() or str(Path.home())
        folder = QFileDialog.getExistingDirectory(
            self,
            t("settings.ooproxy_home_label"),
            current,
        )
        if not folder:
            return
        if not (Path(folder) / "ooproxy.py").exists():
            QMessageBox.warning(self, "ooUI", t("settings.ooproxy_home_invalid"))
            return
        self._home_input.setText(folder)
        self._controller.save_ooproxy_home(folder)
